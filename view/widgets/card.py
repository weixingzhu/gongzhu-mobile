...
class CardWidget(Image):
    def __init__(self, card, size=(CARD_WIDTH, CARD_HEIGHT), **kwargs):
        ...
        image_path = f"assets/cards/{card.key()}.png"
    ...
    def update_status_pos(self, instance, value):
        if self.status_label:
            self.status_label.pos = (
                ...
                self.y + (self.raise_offset if self.is_raised else 0) - 3
            )
    ...
    def update_star_pos(self, instance, value):
        if self.star_label:
            self.star_label.pos = (
                ...
                self.y + self.height - 35 + (self.raise_offset if self.is_raised else 0)
            )
